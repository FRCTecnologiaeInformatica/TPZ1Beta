...
def Historial(usuario,puesto):
  ...
  # ----- Datos Operario Catastral ---- #
    ...
    if proceso_7_o =="Todos":
      ...
      data_1=pd.read_sql(f"select cast(id as integer),marca,usuario,nombre,horario,puesto,supervisor,proceso,fecha,bloque,estado,tipo,cast(predios as integer),cast(horas as float) from registro where usuario='{usuario}'and tipo='{tipo_7_o}' and proceso='{proceso_7_o}' and fecha>='{fecha_de__inicio_7}' and fecha<='{fecha_de__finalizacion_7}'", con)
      data_2 = data_1.groupby(["nombre", "fecha"], as_index=False)["predios","horas"].agg(np.sum)
  ...
  if pivot==0:
    ...
    error= placeholder25_7.error('No existen datos para mostrar')
