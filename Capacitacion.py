...
def Capacitacion(usuario,puesto):
  ...
  if puesto== "Coordinador":
    ...
    filtro_8 = placeholder17_8.selectbox("Filtro", options=("Todos","Operarios","Propio","Personal Asignado"), key="filtro_8")
    ...
    fecha_de__finalizacion_8 = placeholder20_8.date_input("Fecha de Finalización",value=default_date_8,key="fecha_de_finalizacion_8")
      
    data = pd.read_sql(f"select cast(id as integer),marca,usuario,nombre,puesto,supervisor,fecha,tema,horas,reporte from capacitaciones where usuario='{usuario}' and fecha>='{fecha_de__inicio_8}' and fecha<='{fecha_de__finalizacion_8}'", con)

  placeholder21_8 = st.empty()
  histo_8= placeholder21_8.dataframe(data=data)

  placeholder22_8 = st.empty()
  descarga_8 = placeholder22_8.download_button("Decargar CSV",data=data.to_csv(),mime="text/csv",key="descarga_8")
